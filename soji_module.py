...
def pos_servo(dets, frame, client, px=25):
...
def main(clr=clr, send_info=True, debug=True, one=False, model_type='v7t', v7_custom_path="prebuilts/custom/v7t_aqua/V2-512-90%/yolov7.pt", v5_custom_path="C:/Users/GG/Desktop/Code/ML/Models/v5s_70/weights/best.pt", custom_yaml="prebuilts/custom/v7t_aqua/data.yaml", src=0, min_conf=0.5, pixel_threshold=5, record=False, host='localhost', port=8220):
    avg_fps = []
    try:
        if model_type == 'v5m':
            ...
            model = torch.hub.load('WongKinYiu/yolov7', 'custom', "prebuilts/yolov7.pt", verbose=debug)  # YoloV7 Model

        elif model_type == 'v7t':
            model = torch.hub.load('WongKinYiu/yolov7', 'custom', "prebuilts/yolov7t.pt", verbose=debug)  # YoloV7 Tiny Model

        elif model_type == "v7_custom":
            model = torch.hub.load('WongKinYiu/yolov7', 'custom', v7_custom_path, verbose=debug)  # YoloV7 Tiny Model
        ...
        while vid.isOpened():
            ...
            if confs:
                ...
                if send_info:
                    try:
                        pos_servo(detections[most_conf_det], frame, client_socket)
                    except:
